document.addEventListener('DOMContentLoaded', async () => {
	...
	function selectVoice(id, closeList = true) {
		...
		// Handle Custom
		if (id === 'custom') {
			const isDocker = window.SUPERTONIC_TTS_CONFIG?.isDocker || false;
			if (isDocker) {
			...
			document.querySelector('#custom-voice-group label').textContent =
				'Absolute Path to Voice Style JSON or Audio File:';
			voiceFile.type = 'text';
			voiceFile.placeholder = 'C:\\path\\to\\voice_style.json';
		} else {
			customVoiceGroup.classList.add('hidden');
		}
	}
